...
public sealed class IngestionController(
    IIngestionPersistenceService persistence,
    ...
    IIngestionApiKeyValidator apiKey) : ControllerBase
{
    ...
    private async Task<IActionResult> ExecuteAsync<TData, TEntity>(
        ...
        TData data,
        Func<TData, Guid, Guid, (TEntity? entity, Guid id, string? error)> create,
        CancellationToken cancellationToken)
        where TEntity : class
    {
        if (!apiKey.IsValid(Request.Headers["X-Ingestion-Api-Key"].FirstOrDefault()))
        {
            return Unauthorized();
        }
        if (!ValidateSource(source, out var sourceError))
        {
        ...
        var duplicate = await persistence.FindByIdempotencyKeyAsync(
            entityType,
            idempotencyHash,
            cancellationToken);
        ...
        var externalDuplicate = source.ExternalPostId is null
            ? null
            : await persistence.FindByExternalPostAsync(
                entityType,
                source.Platform,
        ...
        var submission = new IngestionSubmission
        {
            ...
            ExternalPostId = source.ExternalPostId?.Trim(),
            IdempotencyKeyHash = idempotencyHash,
            PublishedAt = source.PublishedAt,
            ExtractedAt = source.ExtractedAt ?? DateTimeOffset.UtcNow,
            Confidence = source.Confidence,
            EvidenceSummary = source.EvidenceSummary?.Trim()
        };
        await persistence.CreateSubmissionAsync(created.entity!, submission, cancellationToken);
    ...
    private static (CollectionPoint? entity, Guid id, string? error) CreateCollectionPoint(
        IngestionCollectionPointData data,
        Guid earthquakeId,
        Guid id)
    {
        ...
        {
            return (null, id, "Coordinates must be supplied together and be valid.");
        }
        var point = new CollectionPoint
        {
            ...
            Location = GeoPoint.FromCoordinates(data.Latitude, data.Longitude),
            Description = Trim(data.Description),
            NeedsSummary = data.NeedsSummary.Trim(),
            ReceivingInstructions = data.ReceivingInstructions.Trim(),
            ContactName = Trim(data.ContactName),
            ContactPhone = Trim(data.ContactPhone),
            ContactWhatsApp = Trim(data.ContactWhatsApp),
            ContactEmail = Trim(data.ContactEmail),
            EndsAt = data.EndsAt,
            Source = CollectionPointSource.Automated,
            ModerationStatus = CollectionPointModerationStatus.Pending
        };
    ...
    private static (BloodDonationCenter? entity, Guid id, string? error) CreateBloodDonationCenter(
        IngestionBloodDonationCenterData data,
        Guid earthquakeId,
        Guid id)
    {
        ...
        if (string.IsNullOrWhiteSpace(data.PublicPhone) || data.PublicPhone.Length > 80)
        {
            return (null, id, "Public phone is required.");
        }
        if (!Enum.IsDefined(data.CenterType) || data.BloodTypes == 0 || data.Components == 0)
        {
            return (null, id, "Invalid blood donation data.");
        }
        ...
        {
            return (null, id, "Temporary campaigns require valid dates.");
        }
        var center = new BloodDonationCenter
        {
            ...
            Location = GeoPoint.FromCoordinates(data.Latitude, data.Longitude),
            Description = Trim(data.Description),
            OperatingInstructions = data.OperatingInstructions.Trim(),
            NeedsSummary = data.NeedsSummary.Trim(),
            PublicPhone = data.PublicPhone.Trim(),
            PublicWhatsApp = Trim(data.PublicWhatsApp),
            PublicEmail = Trim(data.PublicEmail),
            CenterType = data.CenterType,
            BloodTypes = data.BloodTypes,
            Components = data.Components,
            StartsAt = data.StartsAt,
            EndsAt = data.EndsAt,
            Source = BloodDonationSource.Automated,
            ModerationStatus = BloodDonationModerationStatus.Pending
        };
    ...
    private static (Shelter? entity, Guid id, string? error) CreateShelter(
        IngestionShelterData data,
        Guid earthquakeId,
        Guid id)
    {
        ...
        var shelter = new Shelter
        {
            ...
            ContactWhatsApp = Trim(data.ContactWhatsApp),
            ContactEmail = Trim(data.ContactEmail),
            Source = ShelterSource.Automated,
            ModerationStatus = ShelterModerationStatus.Pending
        };
    ...
    private static (HelpRequest? entity, Guid id, string? error) CreateHelpRequest(
        IngestionHelpRequestData data,
        Guid earthquakeId,
        Guid id)
    {
        ...
        if (string.IsNullOrWhiteSpace(data.RequesterName) || data.RequesterName.Length > 200)
        {
            return (null, id, "Requester name is required.");
        }
        if (!ValidCoordinates(data.Latitude, data.Longitude))
        {
            return (null, id, "Coordinates must be supplied together and be valid.");
        ...
        if (!Enum.IsDefined(data.Priority) || data.NeedCategories == 0)
        {
            return (null, id, "Priority and at least one need category are required.");
        }
        var request = new HelpRequest
        {
            ...
            Address = data.Address.Trim(),
            Location = GeoPoint.FromCoordinates(data.Latitude, data.Longitude),
            NeedDetails = data.NeedDetails.Trim(),
            ...
            Source = HelpRequestSource.Automated,
            ModerationStatus = HelpRequestModerationStatus.Pending
        };
    ...
    private static string? ValidateCommon(string name, string address, string? description, string requiredText, string? instructions)
    {
        ...
        if (string.IsNullOrWhiteSpace(requiredText) || requiredText.Length > MaxPageText)
        {
            return "Required information is missing or too long.";
        }
        if (description?.Length > 3000 || instructions?.Length > 2500)
        {
            return "Submitted text is too long.";
        }
        return null;
    }
