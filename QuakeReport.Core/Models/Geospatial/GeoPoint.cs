...
public static class GeoPoint
{
    ...
    public static Point? FromCoordinates(double? latitude, double? longitude)
    {
        ...
        if (latitude is null ||
            longitude is null ||
            !IsValid(latitude.Value, longitude.Value))
        {
            throw new ArgumentOutOfRangeException(
                nameof(latitude),
                "Latitude and longitude must be supplied together and be valid WGS84 coordinates.");
        }
    ...
    public static Point FromCoordinates(double latitude, double longitude)
    {
        if (!IsValid(latitude, longitude))
        {
            throw new ArgumentOutOfRangeException(
                nameof(latitude),
                "Coordinates must be valid WGS84 coordinates.");
        }
